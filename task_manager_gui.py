...
class TaskManagerGUI(QWidget):
    ...
    def update_task_list(self):
        self.tasks = select_all_tasks(self.conn)
        task_titles = [f"{task[1]} {task[2]} {task[3]} {task[4]} {task[5]}" for task in self.tasks]
    ...
    def delete_task(self):
        ...
        task_id = next((task[0] for task in self.tasks if task[1] == selected_task_title), None)
